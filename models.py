...
class MappingNetwork(nn.Module):
    def __init__(self, latent_dim=16, style_dim=48, num_domains=2, hidden_dim=384):
        ...
        for _ in range(num_domains):
            ...
            self.unshared += [nn.Linear(hidden_dim, style_dim)]
    ...
    def forward(self, z, y):
        ...
        #     out += [self.unshared[y[i]](h[i])]
        # s = torch.stack(out, dim=0)
        return s
...
class StyleEncoderUni(nn.Module):
    def __init__(self, dim_in=64, style_dim=64, max_conv_dim=512):
        super().__init__()
        blocks = []
        blocks += [nn.Conv2d(1, dim_in, 3, 1, 1)]
...
class Discriminator2d(nn.Module):
    def __init__(self, dim_in=80, num_domains=2, max_conv_dim=384, repeat_num=4):
        super().__init__()
        blocks = []
        blocks += [nn.Conv2d(1, dim_in, 3, 1, 1)]
