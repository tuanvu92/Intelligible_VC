...
def compute_d_loss(nets, args, x_real, y_org, y_trg,
                   z_trg=None, x_ref=None, use_r1_reg=True,
    ...
    loss_con_reg = torch.zeros(1).cuda()
    if use_con_reg:
        t = build_transforms()
    ...
    if use_adv_cls:
        ...
        loss_real_adv_cls = torch.zeros(1).cuda()
        
    loss = loss_real + loss_fake + args.lambda_reg * loss_reg + \
            args.lambda_adv_cls * loss_real_adv_cls + \
            args.lambda_con_reg * loss_con_reg 

    return loss, Munch(real=loss_real,
                       fake=loss_fake,
                       reg=loss_reg,
                       real_adv_cls=loss_real_adv_cls,
                       con_reg=loss_con_reg)
...
def compute_g_loss(nets, args, x_real, y_org, y_trg, z_trgs=None, x_refs=None, use_adv_cls=False):
    ...
    with torch.no_grad():
        ...
        ASR_real = nets.asr_model(x_real)
    ...
    ASR_fake = nets.asr_model(x_fake)
    ...
    if x_refs is not None and args.lambda_f0_sty > 0 and not use_adv_cls:
        ...
        loss_f0_sty = torch.zeros(1).mean().cuda()
    ...
    if args.lambda_asr > 0:
        ASR_recon = nets.asr_model(x_rec)
    ...
    if use_adv_cls:
        ...
        loss_adv_cls = torch.zeros(1).mean().cuda()
    ...
    return loss, Munch(adv=loss_adv,
                       sty=loss_sty,
                       ds=loss_ds,
                       cyc=loss_cyc,
                       norm=loss_norm,
                       ...
                       f0=loss_f0,
                       f0_sty=loss_f0_sty,
                       adv_cls=loss_adv_cls)


def compute_sty_loss(style_net_student, style_net_teacher, x_real, y_org, y_trg, x_refs):
    x_ref, x_ref2 = x_refs        
    
    with torch.no_grad():
        s_real_trg = style_net_teacher(x_real, y_org)
        s_ref_trg = style_net_teacher(x_ref, y_trg)
        s_ref2_trg = style_net_teacher(x_ref2, y_trg)
        
    s_real_pred = style_net_student(x_real)
    s_ref_pred = style_net_student(x_ref)
    s_ref2_pred = style_net_student(x_ref2)
    
    loss = torch.mean(torch.abs(s_real_trg - s_real_pred)) \
           + torch.mean(torch.abs(s_ref_trg - s_ref_pred)) \
           + torch.mean(torch.abs(s_ref2_trg - s_ref2_pred))    

    return loss, Munch(sty=loss)


def finetune_loss(model, style_encoder, x_real, y_org, y_trg, x_refs):
    x_ref, x_ref2 = x_refs
