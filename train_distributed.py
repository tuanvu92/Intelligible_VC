...
from meldataset_distributed import build_dataloader
...
def main(config_path, rank, group_name):
    ...
    if rank == 0:
        ...
        model, _ = build_model(Munch(config['model_params']), F0_model, ASR_model)
        model_ema = None
    ...
    # _ = [model_ema[key].cuda() for key in model_ema]
    for key in model:
        print(key)
        model[key] = apply_gradient_allreduce(model[key].cuda())
        ...
        try:
            ...
            if rank == 0:
                ...
                if (epoch % save_freq) == 0:
                    ...
                    trainer.save_checkpoint(osp.join(log_dir, 'epoch_latest.pth'))
        except KeyboardInterrupt:
            trainer.save_checkpoint(osp.join(log_dir, 'epoch_at_interrupt.pth'))
            sys.exit(0)
    return 0
